...
3. Are there any missing in the dataset?
4. What are the top 20 most common ingredients?
...
* I discovered that the dataset's descriptive parts, such as Ingredient, Unit, and Text, are perfectly intact with zero missing values. In contrast, Rating shows about 50% missing at the row level, while Quantity has no missing values.
...
```{r}
# Clean the duplicate ingredient
cookies1_dedup <- cookies1 %>% distinct(Recipe_Index, Ingredient, .keep_all = TRUE)

# Count the total number of unique recipes
total_recipes_r <- cookies1_dedup %>% summarise(n_distinct(Recipe_Index)) %>% pull()

# Identify the top 20 ingredients and calculate their proportions
top_20_r <- cookies1_dedup %>% 
...
```{python}
# Clean the duplicate ingredient
cookies2_dedup = cookies2[['Recipe_Index', 'Ingredient']].drop_duplicates()

# Count the total number of unique recipes
total_recipes_py = cookies2_dedup['Recipe_Index'].nunique()

# Identify the top 20 ingredients
top_20_py = (
    cookies2_dedup['Ingredient']
    .value_counts()
    .head(20)
    .reset_index()
)
...
# Pull the top 20 most common ingredients
top_20_plot <- top_20_r %>% slice_head(n = 20)

# Build a horizontal bar chart of ingredient proportions
ggplot(top_20_plot, aes(x = reorder(Ingredient, proportion), y = proportion)) +
  geom_col(fill = "steelblue") +
  coord_flip() + 
  labs(title = "Top 20 Ingredients", x = "Ingredient", y = "Proportion") +
  theme_minimal()
```
...
* The plot shows that just a few ingredients are used in most recipes, like salt, flour, baking soda, sugar, and butter. After those, the proportions drop a lot, so many ingredients seem optional. Next, I want to check whether recipes with these less common ingredients have different ratings. 